...
from src.ocr.digit_crops import (
    ...
    partylist_template_vote_cell_crop_box,
...
def test_partylist_template_vote_cell_crop_box_handles_all_table_pages():
    payload = {"page_width": 2480, "page_height": 3509, "zones": [], "lines": []}

    choice_1 = partylist_template_vote_cell_crop_box(
        payload,
        choice_no=1,
        image_width=2480,
        image_height=3509,
    )
    choice_11 = partylist_template_vote_cell_crop_box(
        payload,
        choice_no=11,
        image_width=2480,
        image_height=3509,
    )
    choice_35 = partylist_template_vote_cell_crop_box(
        payload,
        choice_no=35,
        image_width=2480,
        image_height=3509,
    )
    choice_57 = partylist_template_vote_cell_crop_box(
        payload,
        choice_no=57,
        image_width=2480,
        image_height=3509,
    ...
    assert choice_11 is not None
    assert choice_35 is not None
    assert choice_57 is not None
    assert 1200 < choice_1[0] < 1300
    assert 1530 < choice_1[2] < 1600
    assert choice_1[1] > choice_11[1]
...
def test_build_digit_crop_manifest_uses_partylist_template_without_raw_ocr(tmp_path: Path):
    config = ProjectConfig(
        root=tmp_path,
        data={
            "project": {"province": "ชัยภูมิ", "constituency_no": 2},
    ...
    image_dir = tmp_path / "data/raw/images/5_18_partylist/sample"
    processed_dir = tmp_path / "data/processed"
    external_dir = tmp_path / "data/external"
    image_dir.mkdir(parents=True)
    processed_dir.mkdir(parents=True)
    ...
    Image.new("RGB", (2480, 3509), "white").save(image_dir / "sample_page_0001.png")
    (external_dir / "master_parties.csv").write_text(
        "party_no,canonical_name\n35,รักชาติ\n",
        encoding="utf-8",
    )
